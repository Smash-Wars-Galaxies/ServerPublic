...
public:
	...
	void run() {
		...
		try {
			...
			SortedVector<ManagedReference<TreeEntry*> > objects(1024, 1024);
			zone->getInRangeObjects(player->getPositionX(), player->getPositionZ(), player->getPositionY(), 1024, &objects, true);
